...
出力は必ず次の構成だけにしてください。
1行目に最重要点をまとめた要約1文を置き、その後に2行の空行を挟んで、質問への直接的な回答本文を続けます。前置き、ラベル、見出しは付けず、最初の行から要約文を始めてください。
...
回答本文は、質問への結論、基礎的な理由、次に確認する所見の順で構成してください。複雑な鑑別や高度な介入案を広げすぎず、安全に使える基本事項へ絞ってください。"""
...
回答本文は、質問への結論、根拠または機序、評価・介入への具体的な適用、結論を変え得る条件または限界の順で構成してください。すべてを機械的に列挙せず、質問に不要な段階は省略してください。"""
...
class VertexAIClient(BaseClient):
    ...
    def _build_retrieval_tool(
        ...
        if not effective_corpus_id or effective_corpus_id in invalid_corpus_ids:
            raise VertexAIError(
                "対象プランのGOOGLE_CORPUS_IDが未設定です。scripts/setup_rag_corpus.py でコーパスを作成し、"
                "リソース名末尾のIDを設定してください。"
            )
